...
WITH evaluated AS (
    SELECT  p.*,
        ...
        -- ADDED. Does this student exist in student_info AT ALL, on any module?
        -- Separate question from "is the student on THIS module presentation",
        -- and the two have different causes and different fixes.
        -- DISTINCT on one column, so this join cannot multiply rows.
        sany.student_id AS student_exists_anywhere
    ...
    LEFT JOIN (
        ...
        ON p.student_id = sany.student_id),
...
-- INGESTION METADATA
 ...
 -- IS_BANKED
 -- CHANGED. Missing, unparseable and out-of-domain were counted as one
 -- number. They are three different problems with three different fixes,
 -- and every other column in this script already separates them.
        COUNT_IF(
            banked_text IS NULL
        ) AS banked_missing,
        COUNT_IF(
            banked_text IS NOT NULL
            AND (
                banked_value IS NULL
        ...
        -- ADDED. Student has no student_info row on ANY module. Distinct from
        -- enrollment_orphans below, which is a student who exists but is not
        -- on this particular presentation.
        COUNT_IF(
            student_id IS NOT NULL
            AND student_exists_anywhere IS NULL
...
--date submitted
    ...
    FROM tmp_student_assessment_dq_stats s
    UNION ALL
    -- ADDED. Missing is_banked was previously folded into the validity check.
    SELECT
        'COMPLETENESS',
        'Missing is_banked',
        'ROW',
        s.total_records,
        CAST(s.banked_missing AS BIGINT),
        'FAIL',
        TRUE,
        'is_banked must be supplied. It has only two valid values, so a missing one cannot be inferred.'
    FROM tmp_student_assessment_dq_stats s
    UNION ALL
    SELECT
        'VALIDITY',
        ...
        'FAIL',
        TRUE,
        'A supplied is_banked must parse as an integer and equal 0 or 1; missing values are counted separately.'
    FROM tmp_student_assessment_dq_stats s
    UNION ALL
    SELECT
        'COMPLETENESS',
        ...
        -- CHANGED from FAIL. A blank score in OULAD is a NON-SUBMISSION, which
        -- is a real and meaningful state, not a defect. FAIL at threshold 0 made
        -- this permanently red, and a check that is never green gets ignored.
        -- The value is still never treated as zero.
        'WARNING',
        TRUE,
        'A blank score is a non-submission, not a defect. Retained as NULL and never read as zero; the count is reported so its size is known.'
    FROM tmp_student_assessment_dq_stats s
    UNION ALL
    SELECT
        'VALIDITY',
...
-- REFERENCE READINESS
    SELECT   'REFERENTIAL_INTEGRITY',
        ...
        'TABLE',
        -- CHANGED. A TABLE check about the PARENT should report the parent
        -- count, not the child row count.
        s.assessment_parent_count,
        CAST(  CASE
...
'Bronze student_info must provide at least one complete (student, module, presentation) key.'
    ...
    FROM tmp_student_assessment_dq_stats s
    UNION ALL
    -- ADDED. Student missing from student_info ENTIRELY, separate from being
    -- absent on one presentation. This is the population that disappears from
    -- any fact built by joining a student dimension.
    SELECT 'REFERENTIAL_INTEGRITY',
        ...
        'A parseable id_student must appear in Bronze student_info on at least one module presentation. These rows are lost by any downstream join to a student dimension, whatever the module.'
    FROM tmp_student_assessment_dq_stats s
    UNION ALL
    ...
    SELECT 'REFERENTIAL_INTEGRITY',
        ...
        'For rows with a parseable student ID and an unambiguous complete assessment mapping, the student must exist in that module presentation. Students absent from student_info entirely are counted separately.'
    FROM tmp_student_assessment_dq_stats s
),
